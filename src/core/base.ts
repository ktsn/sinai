...
export class Base {
  ...
  constructor(module: ModuleImpl, store: StoreImpl) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      assert(
        proxy !== null,
        'The module proxy is not found in the store, unexpectedly',
      )
    }
...
export interface BA1<S, G extends BG0, M extends BM0>
  extends BA<S, G, M, unknown> {}
